...
import { Copy, Check, Play, CheckCircle, Zap, Shield, Info, Loader2, Sparkles, AlertTriangle } from 'lucide-react';
    ...
    <div className="bg-slate-900/80 border border-slate-800 rounded-xl overflow-hidden shadow-md flex flex-col h-[520px]">
      ...
      {explanation && (
        <div className="bg-slate-950/90 border-t border-slate-800 px-4 py-2 flex items-start space-x-2 text-xs text-slate-400">
          ...
          <span className="leading-relaxed line-clamp-2 flex-1">{explanation}</span>
          {source && (
            <span
              title={sourceDetail || (source === 'gemini' ? 'Generated by the Gemini API' : undefined)}
